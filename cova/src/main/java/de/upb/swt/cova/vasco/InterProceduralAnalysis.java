 ...
 * 
 * <p>
 * This class is a base for forward and backward inter-procedural analysis classes. This inter-procedural analysis framework
 * is fully context sensitive even in the presence of recursion and uses data flow values reaching a method to distinguish
 * contexts.
 * </p>
 * 
 * 
...
public abstract class InterProceduralAnalysis<M, N, A> {
  ...
  /**
   * A flag, if set, directs the analysis to free memory storing data flow values of individual statements once a context has
   * been analysed and would not be required to be re-analysed.
   * 
   * <p>
   * This setting is only useful for analyses that aggregate secondary results on the fly (e.g. call graph analysis that can
   * do away with points-to information once the calls for a particular context have been resolved). This is not safe for use
   * in analyses whose results will be directly used later (e.g. liveness analysis).
   * </p>
   * 
   * <p>
   * Memory is freed when a context is removed from the work-list and no context reachable from it in the transition table is
   * also on the work-list. This ensures that the removed context will not be added again on the work-list for re-analysis of
   * any statement.
   * </p>
   * 
   * <p>
   * Note that the data flow values at the entry/exit of the context are not freed, and hence it is still used to terminate
   * recursion or as a cache hit for arbitrary call sites with same values.
   * </p>
   * 
   * <p>
   * The default value for this flag is <tt>false</tt>.
   * </p>
   * 
   */
   ...
   * 
   * @param reverse
   *          <tt>true</tt> if the analysis is in the reverse direction, <tt>false</tt> if it is in the forward direction
   */
  ...
  /**
   * Returns the initial data flow value at the program entry points. For forward analyses this is the IN value at the ENTRY
   * to each entry method, while for backward analyses this is the OUT value at the EXIT to each entry method.
   * 
   * <p>
   ...
   * 
   * <p>
   * A work-list of contexts is maintained, each with it's own work-list of CFG nodes to process. For each node removed from
   * the work-list of the newest context, the meet of values along incoming edges (in the direction of analysis) is computed
   * and then the flow function is processed depending on whether the node contains a call or not. If the resulting data flow
   * value has changed, then nodes along outgoing edges (in the direction of analysis) are also added to the work-list.
   * </p>
   * 
   * <p>
   * Analysis starts with the context for the program entry points with the given boundary values and ends when the work-list
   * is empty.
   * </p>
   * 
   * <p>
   ...
   * 
   * @param method
   *          the method whose value context to find
   * @param value
   *          the data flow value at the entry (forward flow) or exit (backward flow) of the method
   * @return the value context, if one is found with the given parameters, or <tt>null</tt> otherwise
   */
   ...
   * 
   * <p>
   * This method should not be invoked if the flag {@link #freeResultsOnTheFly} had been set during analysis.
   * </p>
   * 
   ...
   *
   * @param local
   *          the local value of caller
   * @param ret
   *          the exit value of callee
   * @return the a
   */
  public abstract A merge(A local, A ret);

  /**
   * Returns a program representation on top of which the inter-procedural analysis runs. The program representation is used
   * to build control flow graphs of individual procedures and resolve targets of virtual call sites.
   * 
