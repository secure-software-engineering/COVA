...
public class WitnessPath {
  ...
  public void add(NodeType type, String className, int lineNumber) {
    ...
    if (!this.positions.contains(str)) this.positions.add(b.toString());
  }
  ...
  public void addAll(List<String> positions) {
    for (String position : positions) this.add(position);
  }

  public static WitnessPath merge(WitnessPath p1, WitnessPath p2) {
    WitnessPath p = new WitnessPath();
    for (String s : p1.positions) if (!p.positions.contains(s)) p.positions.add(s);
    for (String s : p2.positions) if (!p.positions.contains(s)) p.positions.add(s);
    return p;
  }
  ...
  public String toString() {
    ...
    for (int i = 0; i < this.positions.size(); i++) {
      ...
      if (i != this.positions.size() - 1) b.append("\n\t->");
    }
    return b.toString();
  }
