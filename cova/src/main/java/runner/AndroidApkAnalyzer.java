...
import core.Aliasing;
import core.SMTSolverZ3;
import data.CombinedResult;
import data.MetaData;
import reporter.ConstraintReporter;
import reporter.ResultPrinter;
import setup.RunCova;
import setup.RunFlowDroid;
...
public class AndroidApkAnalyzer {
  ...
  private static void analyze(File apk, Config config) throws IOException {
    try {
      boolean timedout = analyzeApk(androidJarPath, apk.getCanonicalPath(), sourceCodePath,
          standalone,
          config);
      System.gc();
  ...
  public static boolean analyzeApk(String androidJarPath, String apkFilePath, String sourceCodePath,
      boolean standalone, Config config) {
    try {
      ...
      if (Files.getFileExtension(apkFilePath).equals("apk")) {
        if (!standalone) {
          if (!outputCSV) {
            ...
            // run cova
            reporter = RunCova.runForAndroid(apkFilePath,
                androidJarPath, sourceCodePath, standalone, RunFlowDroid.getCallbacks(), config);
            if (infoFlowResults.getResults() != null) {
              CombinedResult combinedResults = new CombinedResult(null,
                  infoFlowResults.getResults(), reporter);
              results = combinedResults;
              combinedResults.print();
            } else {
            ...
            ConstraintReporter reporter = RunCova.runForAndroid(apkFilePath,
                androidJarPath, sourceCodePath, standalone, RunFlowDroid.getCallbacks(), config);
            ...
            MetaData meta = new data.MetaData(apkName, size, reachableMethods, ftime, timeout,
                covaTime, z3Time, failedAliasing, z3Queries);
            // print results in .txt
            if (infoFlowResults.getResults() != null) {
              CombinedResult combinedResults = new CombinedResult(meta,
                  infoFlowResults.getResults(), reporter);
              results = combinedResults;
