...
public class TaintConstraintCreationRule implements IRule<SootMethod, Unit, Abstraction> {
   ...
   */
  private Abstraction createConstraintForIfStmt(
        ...
        for (AbstractTaint taint2 : involved2) {
          IConstraint c =
              ConstraintFactory.createConstraint(
                  ruleManager.getConfig().recordPath(),
                  taint1,
                  taint2,
                  conditionExpr,
                  isFallThroughEdge);
   ...
   */
  private Abstraction createConstraintForTableSwitchStmt(
      Context<SootMethod, Unit, Abstraction> context, Unit node, Unit succ, Abstraction in) {
    IConstraint constraint = null;
            ...
            if (createConstraint) {
              ...
              for (int i = 0; i < switchStmt.getTargets().size(); i++) {
                if (!defaultTarget.equals(switchStmt.getTarget(i))) {
                  ...
                  IConstraint c =
                      ConstraintFactory.createConstraint(
                          ruleManager.getConfig().recordPath(), taint, eq, false, true);
                  if (ruleManager.getConfig().recordPath()) {
                    c.getPath()
                        .add(
                            NodeType.control,
   ...
   */
  private Abstraction createConstraintForLookupSwitchStmt(
      Context<SootMethod, Unit, Abstraction> context, Unit node, Unit succ, Abstraction in) {
